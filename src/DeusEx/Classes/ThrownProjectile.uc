...
function PlayBeepSound( float Range, float Pitch, float volume )
{
	if ( Level.NetMode != NM_Standalone )
		PlaySound( sound'Beep4',SLOT_None, volume,, Range, Pitch * DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier() );
	else
		PlaySound( sound'Beep4',SLOT_None,,, Range, Pitch * DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier() );
}

function float CalculateSkillTime()
{
	return FClamp(-20.0 * DeusExPlayer(GetPlayerPawn()).SkillSystem.GetSkillLevelValue(class'SkillWeaponHeavy'), 1.0, 10.0);
}
					...
					if (Owner == None)
					{
						foreach RadiusActors(class'DeusExPlayer', Player, proxRadius*4)
						{
							...
							if (Player != Owner)
							{
								...
								if ((VSize(dist) < proxRadius) && !Player.IsInvisibleForRobots())
									if (skillTime == 0)
										skillTime = CalculateSkillTime();
							}
						}
					}
					else
								...
								if ( curPawn.IsA('DeusExPlayer') )
								{
									...
									if ( !pass )
									{
										...
										if ( VSize(dist) < proxRadius )
										{
											if (skillTime == 0)
											{
												skillDiff = -skillAtSet + Player.SkillSystem.GetSkillLevelValue(class'SkillWeaponHeavy');
												if ( skillDiff >= 0.0 ) // Scale goes 1.0, 1.6, 2.8, 4.0
													skillTime = FClamp( 1.0 + skillDiff * 6.0, 1.0, 2.5 );
												else	// Scale goes 1.0, 1.4, 2.2, 3.0
													skillTime = FClamp( 1.0	+ (-Player.SkillSystem.GetSkillLevelValue(class'SkillWeaponHeavy') * 4.0), 1.0, 3.0 );
											}
										}
									}
								}
...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	else if (bProximityTriggered && bArmed && (skillTime >= 0))
	{
		...
		PlaySound(sound'Beep4',SLOT_None,,, 1280, 0.5 * DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
		bDisabled = True;
	}
}
...
simulated function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, name damageType)
{
	...
	if ( Role == ROLE_Authority )
	{
		...
		if (DamageType == 'EMP')
		{
			if (!bDisabled)
			{
				PlaySound(sound'EMPZap', SLOT_None,,, 1280, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
				bDisabled = True;
				...
				if (gen != None)
				{
					...
					gen.particleTexture = Texture'GameMedia.Effects.ef_ExpSmoke007';
					gen.SetBase(Self);
				}
			}
			return;
...
{
	PlaySound(ImpactSound, SLOT_None, 2.0,, blastRadius*16, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
}
...
auto simulated state Flying
{
	...
	simulated function HitWall (vector HitNormal, actor HitWall)
	{
		...
		if ( (speed < 60) && (HitNormal.Z > 0.7) )
		{
			...
			PlaySound(MiscSound, SLOT_None, volume,, 512, (0.85+FRand()*0.3) * DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
	...
	if ( NewZone.bWaterZone )
	{
		...
		if ( Level.NetMode != NM_DedicatedServer )
		{
			if ( NewZone.EntrySound != None )
				PlaySound(NewZone.EntrySound, SLOT_None, splashSize,,, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
...
simulated function BeginPlay()
{
	...
	if (( Level.NetMode != NM_Standalone ) && (Role == ROLE_Authority))
	{
		...
		skillAtSet = aplayer.SkillSystem.GetSkillLevelValue(class'SkillWeaponHeavy');
	}
