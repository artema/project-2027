...
{		
	...
	gc.SetFont(Font'FontBarkName');
	gc.SetAlignments(HALIGN_Center, VALIGN_Top);
	gc.EnableTranslucentText(True);

	if (bButtonPressed)
		gc.DrawText(1, 1-2, width, height, parent.IndexToString(num));
	else
		gc.DrawText(0, 0-2, width, height, parent.IndexToString(num));
	
	gc.SetFont(Font'Engine.SmallFont');
	gc.SetAlignments(HALIGN_Center, VALIGN_Top);
	gc.EnableTranslucentText(True);
		
	if (bButtonPressed)
		gc.DrawText(1-2, height-11, width+4, height, parent.IndexToHint(num));
	else
		gc.DrawText(0-2, height-11-1, width+4, height, parent.IndexToHint(num));
}
