...
var localized String ChargePlanted;
...
function CreateControls()
{
	...
	CreateTitleWindow(9, 4, InventoryTitleText);
	CreateInfoWindow();
	CreateCreditsWindow();
	CreateObjectBelt();
	CreateButtons();
...
function CreateButtons()
{
	...
	winActionButtons.SetWidth(267);//267
	...
	if (buttonPressed.IsA('PersonaAmmoDetailButton'))
	{
		if (DeusExWeapon(selectedItem.GetClientObject()) != None)
		{
			...
			if (!PersonaAmmoDetailButton(buttonPressed).bSelected)
			{
				...
				DeusExWeapon(selectedItem.GetClientObject()).UpdateInfo(winInfo);
				EnableButtons();
			}
		}
	}
...
function UseSelectedItem()
{
	...
	if (inv != None)
	{
		if (inv.IsA('SupportBotItem') || inv.IsA('SpyDroneItem'))
			return;
	...
	if (Inventory(selectedItem.GetClientObject()) != None)
	{
		...
		if (!Inventory(selectedItem.GetClientObject()).IsA('NanoKeyRing'))
		{
			...
			else
			{
                ...
                if (player.Level.NetMode == NM_Standalone)	
			winStatus.AddText(Sprintf(CannotBeDroppedLabel));			
...
function EnableButtons()
{
	...
	if ((btnChangeAmmo == None) || (btnDrop == None) || (btnUse == None))
		return;
	...
	else
	{
		...
		if (inv != None)
		{
			...
			else if (inv.IsA('WeaponTool'))
			{
				btnChangeAmmo.DisableWindow();
				btnUse.DisableWindow();		
			}
			else if (inv.IsA('SupportBotItem'))
			{
				btnChangeAmmo.DisableWindow();
				btnUse.DisableWindow();		
			}
			else if (inv.IsA('SpyDroneItem'))
			{
				btnChangeAmmo.DisableWindow();
				btnUse.DisableWindow();		
			}
...
function UpdateDragMouse(float newX, float newY)
{
	...
	if (dragButton.IsA('PersonaInventoryItemButton'))
	{
		...
		else if (PersonaInventoryItemButton(findWin) != None)
		{
			...
			else if ((dragButton.GetClientObject().IsA('WeaponTool')) && (findWin.GetClientObject().IsA('SupportBotItem')))
			{
				if (WeaponTool(invButton.GetClientObject()).CanFixItem(SupportBotItem(findWin.GetClientObject())))
				{
					bValidDrop = True;
					PersonaInventoryItemButton(findWin).SetDropFill(True);
					invButton.bValidSlot = False;
			...
			else if ((dragButton.GetClientObject().IsA('BioelectricCell')) && (findWin.GetClientObject().IsA('SupportBotItem')))
			{
				if (BioelectricCell(invButton.GetClientObject()).CanChargeItem(SupportBotItem(findWin.GetClientObject())))
				{
					bValidDrop = True;
					PersonaInventoryItemButton(findWin).SetDropFill(True);
					invButton.bValidSlot = False;
			...
			else if ((DeusExWeapon(dragButton.GetClientObject()).bIsGrenade) && (findWin.GetClientObject().IsA('SupportBotItem')))
			{
				/*if(Player.PerkSystem.CheckPerkState(class'PerkBotexplode')){
				if (!SupportBotItem(findWin.GetClientObject()).HasSpawnGrenade())
				{
					bValidDrop = True;
					PersonaInventoryItemButton(findWin).SetDropFill(True);
					invButton.bValidSlot = False;
			...
			else if ((dragButton.GetClientObject().IsA('BioelectricCell')) && (findWin.GetClientObject().IsA('SpyDroneItem')))
			{
				if (BioelectricCell(invButton.GetClientObject()).CanChargeItem(SpyDroneItem(findWin.GetClientObject())))
				{
					bValidDrop = True;
					PersonaInventoryItemButton(findWin).SetDropFill(True);
					invButton.bValidSlot = False;
			...
			else if ((DeusExWeapon(dragButton.GetClientObject()).bIsGrenade) && (findWin.GetClientObject().IsA('SpyDroneItem')))
			{
				/*if(Player.PerkSystem.CheckPerkState(class'PerkBotexplode')){
				if (!SpyDroneItem(findWin.GetClientObject()).HasSpawnGrenade())
				{
					bValidDrop = True;
					PersonaInventoryItemButton(findWin).SetDropFill(True);
					invButton.bValidSlot = False;
...
function FinishButtonDrag()
{
	...
	local string ApplyMSG;
	local class<Projectile> spawnclass;
	...
	{	
		...
		else if ( (dragInv.IsA('WeaponTool')) && (dragTarget != None) && (dragTarget.GetClientObject().IsA('SupportBotItem')) )
		{
			if (WeaponTool(dragInv).CanFixItem(SupportBotItem(dragTarget.GetClientObject())))
			{
				WeaponTool(dragInv).ApplyTool(SupportBotItem(dragTarget.GetClientObject()));
				Player.RemoveObjectFromBelt(dragInv);
				ApplyMSG = WeaponTool(dragInv).GetApplyMessage();
				winStatus.AddText(Sprintf(ApplyMSG, SupportBotItem(dragTarget.GetClientObject()).itemName));
				PlaySound(ToolSound, 255);
				WeaponTool(draginv).DestroyTool();
				dragButton = None;
				SelectInventory(dragTarget);
			}
			else
		...
		else if ( (dragInv.IsA('BioelectricCell')) && (dragTarget != None) && (dragTarget.GetClientObject().IsA('SupportBotItem')) )
		{
			if (BioelectricCell(dragInv).CanChargeItem(SupportBotItem(dragTarget.GetClientObject())))
			{
				BioelectricCell(dragInv).ApplyCell(SupportBotItem(dragTarget.GetClientObject()));
				Player.RemoveObjectFromBelt(dragInv);
				ApplyMSG = BioelectricCell(dragInv).GetApplyMessage();
				winStatus.AddText(Sprintf(ApplyMSG, SupportBotItem(dragTarget.GetClientObject()).itemName, BioelectricCell(dragInv).rechargeAmount));
				PlaySound(BioelectricCell(dragInv).ChargeSound, 255);
				BioelectricCell(draginv).DestroyCell();
				dragButton = None;
				SelectInventory(dragTarget);
			}
			else
		...
		else if ((dragInv.IsA('DeusExWeapon')) && (DeusExWeapon(dragInv).bIsGrenade) && (dragTarget != None) && (dragTarget.GetClientObject().IsA('SupportBotItem')) )
		{
			/*if(Player.PerkSystem.CheckPerkState(class'PerkBotexplode'))
			{
				if (!SupportBotItem(dragTarget.GetClientObject()).HasSpawnGrenade())
				{
					SupportBotItem(dragTarget.GetClientObject()).SetSpawnGrenade(DeusExWeapon(dragInv).GrenadeNum);
					//Player.RemoveObjectFromBelt(dragInv);
					winStatus.AddText(ChargePlanted);
					PlaySound(ToolSound, 255);
					DeusExWeapon(dragInv).DecreaseAmmo();
					dragButton = None;
					SelectInventory(dragTarget);
				}
				else
		...
		else if ( (dragInv.IsA('BioelectricCell')) && (dragTarget != None) && (dragTarget.GetClientObject().IsA('SpyDroneItem')) )
		{
			if (BioelectricCell(dragInv).CanChargeItem(SpyDroneItem(dragTarget.GetClientObject())))
			{
				BioelectricCell(dragInv).ApplyCell(SpyDroneItem(dragTarget.GetClientObject()));
				Player.RemoveObjectFromBelt(dragInv);
				ApplyMSG = BioelectricCell(dragInv).GetApplyMessage();
				winStatus.AddText(Sprintf(ApplyMSG, SupportBotItem(dragTarget.GetClientObject()).itemName, BioelectricCell(dragInv).rechargeAmount));
				PlaySound(BioelectricCell(dragInv).ChargeSound, 255);
		...
		else if ((dragInv.IsA('DeusExWeapon')) && (DeusExWeapon(dragInv).bIsGrenade) && (dragTarget != None) && (dragTarget.GetClientObject().IsA('SpyDroneItem')) )
		{
			/*if(Player.PerkSystem.CheckPerkState(class'PerkBotexplode'))
			{
				if (!SpyDroneItem(dragTarget.GetClientObject()).HasSpawnGrenade())
				{
					SpyDroneItem(dragTarget.GetClientObject()).SetSpawnGrenade(DeusExWeapon(dragInv).GrenadeNum);
					//Player.RemoveObjectFromBelt(dragInv);
					winStatus.AddText(ChargePlanted);
					PlaySound(ToolSound, 255);
					DeusExWeapon(dragInv).DecreaseAmmo();
...
function HighlightSpecial(Inventory item)
{
	if (item != None)
	{
		...
		else if (item.IsA('WeaponTool'))//!!1
			HighlightToolItems(WeaponTool(item));
		else if (item.IsA('BioelectricCell'))//!!1
			HighlightCellItems(BioelectricCell(item));
		else if (DeusExWeapon(item).bIsGrenade)//!!1
			HighlightGrenadeItems(DeusExWeapon(item));
...
// ----------------------------------------------------------------------
//!!1
function HighlightToolItems(WeaponTool weaponTool)
{
	local Window itemWindow;
	local PersonaInventoryItemButton itemButton;
	local Inventory anItem;
	...
	while(itemWindow != None)
	{
		...
		if (itemButton != None)
		{
			...
			if ((anItem != None) && (anItem.IsA('SupportBotItem')))
			{
				if ((weaponTool != None) && (weaponTool.CanFixItem(anItem)))
				{
					itemButton.HighlightItem(True);
				}
			}
			else
			{
...
// ----------------------------------------------------------------------
// HighlightGrenadeItems()
// ----------------------------------------------------------------------
//!!1
function HighlightGrenadeItems(DeusExWeapon wep)
{
	local Window itemWindow;
	local PersonaInventoryItemButton itemButton;
	local Inventory anItem;
	...
	while(itemWindow != None)
	{
		...
		if (itemButton != None)
		{
			...
			if ((anItem != None) && (anItem.IsA('SupportBotItem')))
			{
				if ((wep != None) && (wep.bIsGrenade) && (!SupportBotItem(anItem).HasSpawnGrenade()))
				{
					itemButton.HighlightItem(True);
				}
			}
			else if ((anItem != None) && (anItem.IsA('SpyDroneItem')))
			{
				if ((wep != None) && (wep.bIsGrenade) && (!SpyDroneItem(anItem).HasSpawnGrenade()))
				{
					itemButton.HighlightItem(True);
				}
			}
...
// ----------------------------------------------------------------------
//!!1
function HighlightCellItems(BioelectricCell Cell)
{
	local Window itemWindow;
	local PersonaInventoryItemButton itemButton;
	local Inventory anItem;
	...
	while(itemWindow != None)
	{
		...
		if (itemButton != None)
		{
			...
			if ((anItem != None) && (anItem.IsA('SupportBotItem')))
			{
				if ((Cell != None) && (Cell.CanChargeItem(anItem)))
				{
					itemButton.HighlightItem(True);
				}
			}
...
defaultproperties
{
     ToolSound=Sound'DeusExSounds.Weapons.ProdReload'
     ModSound=Sound'DeusExSounds.Weapons.ProdReload'
     invButtonWidth=53
     invButtonHeight=53
     smallInvWidth=40
     smallInvHeight=35
