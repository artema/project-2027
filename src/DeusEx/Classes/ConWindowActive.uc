...
var localized Color colConTextFocus;
var localized Color colConTextChoice;
var localized Color colConTextSkill;
...
var ConWindowCredits winCredits;
...
event InitWindow()
{
	...
	upperConWindow.SetBackgroundStyle(DSTY_Normal);
	// Create upper window
...
function CalculateWindowSizes()
{
	...
	if (bForcePlay)
	{
		...
		cinHeight = min(root.height * ratio, root.height * 0.75);
...
function ShowCredits(int count)
{
	if (winCredits == None)
	{
		winCredits = ConWindowCredits(upperConWindow.NewChild(Class'ConWindowCredits'));
		winCredits.SetCreditsCount(count);
	}
}
