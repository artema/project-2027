	...
	if (player != None)
	{
		...
		if (frobTarget != None)
		{
			...
			if (frobTarget.IsA('Mover'))
			{
				...
				if ((dxMover != None) && dxMover.bLocked)
				{
					...
					if (dxMover.bBreakable)
						strInfo = strInfo $ dxMover.minDamageThreshold;
				...
				if ((dxMover != None) && dxMover.bLocked)
				{
					...
					if(dxMover.origDoorStrength > 0.0)
						gc.DrawPattern(infoX+(infoW-barLength-4), infoY+4+2*(infoH-8)/3, barLength*(dxMover.doorStrength/dxMover.origDoorStrength), ((infoH-8)/3)-2, 0, 0, Texture'ConWindowBackground');
				}
				...
				{
					numTools = int((dxMover.lockStrength / player.SkillSystem.GetSkillLevelValue(class'SkillTechnics')) + 0.99);
					if (numTools == 1)
			...
			else if (frobTarget.IsA('HackableDevices'))
			{
				...
				{
					numTools = int((device.hackStrength / player.SkillSystem.GetAltSkillLevelValue(class'SkillElectronics'))  + 0.99);
					if (numTools == 1)
