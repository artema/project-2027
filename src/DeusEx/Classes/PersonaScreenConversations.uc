...
function CreateControls()
{
	...
	CreateTitleWindow(9, 4, ConversationsTitleText);
...
function CreateHistoryListWindow()
{
	...
	lstCons.SetColumnWidth(0, 121);
	lstCons.SetColumnWidth(1, 234);
	lstCons.SetColumnWidth(2, 30);//40
	lstCons.SetColumnFont(2, Font'FontHUDWingDings');
	lstCons.SetColumnAlignment(2, HALIGN_Right);
}
...
function CreateButtons()
{
	...
	btnLocation = CreateBarButton(winButtonBar, 234, LocationButtonLabel);
	btnActor    = CreateBarButton(winButtonBar, 122, ActorButtonLabel);
}
...
function DisplayHistory(ConHistory history)
{
	...
	local PersonaNormalBigTextWindow txtSpeech;
	...
	while(event != None)
	{
		...
		txtSpeech = PersonaNormalBigTextWindow(conWindow.NewChild(Class'PersonaNormalBigTextWindow'));
