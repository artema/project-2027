...
function FirstFrame()
{
	...
	Player.SetLoadScr(dxInfo.CurrentMapScreen);
	
	if(bAutosave && !flags.GetBool('GameAutosaved'))
	{
		flags.SetBool('GameAutosaved', True,, dxInfo.MissionNumber + 1);
		Player.AutoSaveGame(AutosaveName);
	}
}
...
defaultproperties
{
     checkTime=0.500000
     localURL="NOTHING"
     AutosaveName="DEFAULT NAME - REPORT THIS AS A BUG"
}
