...
function Timer()
{
	...
	if (bPicking)
	{
		...
		if(!DeusExPlayer(GetPlayerPawn()).bPaused)
	  		//TicksSinceLastPick += (Level.TimeSeconds - LastTickTime) * 10;
	  		TicksSinceLastPick += 1;
		...
		if (frobWith != None)
		{
			...
			if (bPickable && frobWith.IsA('Lockpick') && (Player.SkillSystem != None))
			{
				if (bLocked)
				{
					...
					pickValue = Player.SkillSystem.GetSkillLevelValue(class'SkillTechnics');
					pickPlayer = Player;
