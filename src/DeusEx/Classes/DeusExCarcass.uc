...
var localized string msgHasKnife;
var localized string msgHasCrowbar;
var localized string msgHasBaton;
...
var String			AliveBindName;

var bool bGibbed;

var HUDCarcassWindow carcasswindow;
	...
	for (i=0; i<8; i++)
	{
		if ((InitialInventory[i].inventory != None) && (InitialInventory[i].count > 0))
		{
			for (j=0; j<InitialInventory[i].count; j++)
			{
				...
				if (inv != None)
				{
					if(!inv.IsA('DeusExWeapon') || (inv.IsA('DeusExWeapon') && !DeusExWeapon(inv).bCannotBePickedUp))
					{
						inv.bHidden = True;
...
function bool PickUpCorpse(DeusExPlayer player)
{
	...
	if (corpse != None)
	{
		...
		{
			nextItem = item.Inventory;
			if(item != None) item.SetOwner(corpse);
			item = nextItem;
		}
		until ((item == None) || (item == startItem));
		corpse.TempInventory = Inventory;
		Inventory = None;
...
function ShowCorpseWindow(DeusExPlayer player)
{
	...
	if (root != None)
	{
		carcasswindow = HUDCarcassWindow(root.InvokeUIScreen(Class'HUDCarcassWindow', True));
		root.MaskBackground(True);
      
		if (carcasswindow != None)
		{
			carcasswindow.carcassOwner = Self;
			carcasswindow.player = player;
			carcasswindow.InitData();
		}
	}
}
		...
		if (Inventory != None)
		{
			...
			do
			{
				...
				if (item.IsA('Ammo'))
				{
					...
					{
						if(item.IsA('RAmmoSabot'))
							itemCount = 1 + Rand(12);
						else if(item.IsA('RAmmo10mmJHP'))
							itemCount = 1 + Rand(12);
						else if(item.IsA('RAmmo556mmJHP'))
							itemCount = 1 + Rand(30);
						else if(item.IsA('RAmmo762mmB'))
							itemCount = 1 + Rand(30);
						else if(item.IsA('RAmmoRocket'))
							itemCount = 1 + Rand(4);
						else
							itemCount = 1 + Rand(4);
						...
						if(player.FindInventoryType(item.Class) != None)
						{
							...
							{
								//if(item.IsA('RAmmoShuriken'))
								//	item = player.FindInventoryType(Class'WeaponThrowingKnife');
							}
						...
						if ((W != None) || ((W == None) && (!player.FindInventorySlot(item, True))) && !bPickedItemUp)
						{
							...
							if ((Weapon(item).AmmoType != None) && (Weapon(item).AmmoType.AmmoAmount > 0))
							{
                        		...
                        		if ((AmmoType != None) && (AmmoType.AmmoAmount < AmmoType.MaxAmmo))
								{
									...
									if(item.IsA('WeaponThrowingKnife'))
