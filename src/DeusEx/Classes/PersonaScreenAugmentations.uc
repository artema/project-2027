...
var Localized string AugLocationDesc[6];
var PersonaHealthItemButton     selectedAugInfoButton;
...
function CreateControls()
{
	...
	CreateTitleWindow(9, 4, AugmentationsTitleText);
	CreateInfoWindow();
	CreateButtons();
...
{
	CreateLabel( 57,  26, AugLocationCranial);
	CreateLabel(212,  26, AugLocationEyes);
	CreateLabel( 19, 102, AugLocationArms);
	CreateLabel( 19, 186, AugLocationSubdermal);
	CreateLabel(247, 108, AugLocationTorso);
	CreateLabel( 72, 328, AugLocationDefault); //19-330
	CreateLabel(247, 310, AugLocationLegs);
}
...
function RefreshWindow(float DeltaTime)
{
    ...
    {
        PersonaAugmentationItemButton(selectedAugButton).SetLevel(selectedAug.GetCurrentLevel(), selectedAug.MaxLevel);
...
function CreateAugmentationHighlights()
{
	...
	CreateHighlight2(142,  45, 16, 19, AugLocationDesc[0], AugLocationCranial);
	CreateHighlight2(161,  63, 19, 12, AugLocationDesc[1], AugLocationEyes);
	CreateHighlight2(157, 108, 34, 48, AugLocationDesc[2], AugLocationTorso);
	CreateHighlight2(105, 110, 24, 43, AugLocationDesc[3], AugLocationArms);
	CreateHighlight2(165, 222, 32, 94, AugLocationDesc[4], AugLocationLegs);
	CreateHighlight2(84, 160, 14, 36, AugLocationDesc[5], AugLocationSubdermal);
}
	...
	String LocName)
{
	...
	newPart.SetDesc(LocDesription);
	newPart.SetTitle(LocName);
	newPart.Raise();
}
...
function PersonaAugmentationItemButton CreateAugButton(Augmentation anAug, int augX, int augY, int slotIndex)
{
	...
	newButton.SetLevel(anAug.GetCurrentLevel(), anAug.MaxLevel);
...
function bool ButtonActivated(Window buttonPressed)
{
	...
	{
		SelectAugPart(PersonaHealthItemButton(buttonPressed));
	}
	else if (buttonPressed.IsA('PersonaItemButton') && !buttonPressed.IsA('PersonaHealthItemButton'))
	{
...
function SelectAugPart(PersonaHealthItemButton buttonPressed)
{
	// Don't do extra work.
	if (selectedAugInfoButton != buttonPressed)
	{
		// Deselect current button
		if (selectedAugInfoButton != None)
			selectedAugInfoButton.SelectButton(False);

		selectedAugInfoButton = buttonPressed;
		selectedAugInfoButton.SelectButton(True);

		// Update the display
		winInfo.SetTitle(selectedAugInfoButton.GetTitle());
		winInfo.SetText(selectedAugInfoButton.GetDesc());
