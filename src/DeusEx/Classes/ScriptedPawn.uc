...
var() sound HelmetHitSound;
var()	bool bHasHelmet;
var()	bool bHasMask;
var()	bool bHasLegDefence;
var()	bool bHasTorsoDefence;

var() int MeleeDamageThreshold;

var bool bMetal;

var() bool bRunToAttackingEnemy;

var float customVisibility;

var(Combat) bool bExplodingCorpse;
var(Combat) int AmritaRate;
var float amritaTimer;

var(Combat) float EmpFieldRadius;
var float empTimer;

var(Combat) bool bUseMaxRange;
...
function Pawn InstigatorToPawn(Actor eventActor)
{
	...
	else if(GrenadeProjectile(eventActor) != None && !GrenadeProjectile(eventActor).bEmitWeaponShot)
		eventActor = None;
...
function GenerateTotalHealth()
{
	...
	if (!bInvincible)
	{
		...
		limbDamage *= 0.2;  // 2 1/2 limbs disabled == death
...
function UpdateAmrita(float deltaTime)
{
	if (AmritaRate <= 0 || (Health <= 0) || Health >= Default.Health || bDeleteMe)  // no more pain -- you're already dead!
		return;

	amritaTimer += deltaTime;
...
function float ComputeActorVisibility(Actor seeActor)
{
	...
	else if (seeActor.IsA('ScriptedPawn') && ScriptedPawn(seeActor).bCloakOn)
	{
		visibility = 0.0;
	}
	else if(customVisibility > 0.0)
	{
...
function bool IsProjectileDangerous(DeusExProjectile projectile)
{
	...
	else if (projectile.IsA('ThrownProjectile'))
	{
		if (projectile.IsA('SpyBot'))
			...
			bEvil = false;
		else if(projectile.IsA('GrenadeProjectile') && !GrenadeProjectile(projectile).bEmitWeaponShot)
			bEvil = false;
		else
			bEvil = true;
	}
...
function Carcass SpawnCarcass()
{
	...
	if (((Health < -100) && !IsA('Robot')) || (bExplodingCorpse && !bStunned))
	{
		...
		{	
			if(!IsA('Animal'))
			{
				if(FRand() < 0.75)
				{
					...
					chunk = spawn(class'FleshFragmentHead', None,, loc);
					if (chunk != None)
					{
						chunk.bFixedRotationDir = True;
						chunk.RotationRate = RotRand(False);
						chunk.AddRandomSmoke();
					}
				}
				...
				for (i=0; i<2; i++)
				{
					if(FRand() < 0.75)
					{
						...
						chunk = spawn(class'FleshFragmentFoot', None,, loc);
						if (chunk != None)
						{
							chunk.bFixedRotationDir = True;
							chunk.RotationRate = RotRand(False);
					...
					if(FRand() < 0.75)
					{
						...
						chunk = spawn(class'FleshFragmentLeg', None,, loc);
						if (chunk != None)
						{
							chunk.bFixedRotationDir = True;
							chunk.RotationRate = RotRand(False);
					...
					if(FRand() < 0.75)
					{
						...
						chunk = spawn(class'FleshFragmentForearm', None,, loc);
						if (chunk != None)
						{
							chunk.bFixedRotationDir = True;
							chunk.RotationRate = RotRand(False);
			...
			if(FRand() < 0.5)
			{
				...
				chunk = spawn(class'FleshFragmentGuts', None,, loc);
				if (chunk != None)
				{
					chunk.bFixedRotationDir = True;
					chunk.RotationRate = RotRand(False);
			...
			for (i=0; i<size/2.0; i++)
			{
				...
				loc.Z = (1-2*FRand()) * CollisionHeight;
				loc += Location;
				chunk = spawn(class'FleshFragmentGib', None,, loc);
				if (chunk != None)
				{
					chunk.DrawScale = size / 20;
		...
		carc.bHidden       = true;
		carc.bDetectable   = false;
		carc.SetCollision(false, false, false);
		carc.bCollideWorld = false;
		carc.SetPhysics(PHYS_None);
		carc.SetLocation(Location+vect(0,0,20000));
		carc.bGibbed = True;

		if(bExplodingCorpse && !bStunned)
		{
			SelfExplode();
		}
	...
	if (actualDamage > 0)
	{
		if (offset.z > headOffsetZ)		// head
		{
			...
			if ((Abs(offset.x) < headOffsetY) || (Abs(offset.y) < headOffsetY))
			{
				...
				if ((damageType == 'Stunned') || (damageType == 'KnockedOut')){
					if(!bHasHelmet && !bHasMask)
						HealthHead -= actualDamage;
				}
				else
				{
					if(hitPos == HITLOC_HeadBack)
					{
						if(bHasHelmet){
							if(actualDamage >= 22)
								HealthHead -= actualDamage * 3.5;
							else if(actualDamage < 22 && actualDamage >= 18)
								HealthHead -= actualDamage * 3;
							else if(actualDamage < 18)
					...
					else if(hitPos == HITLOC_HeadFront)
					{
						if(bHasMask && actualDamage < 18)
							HealthHead -= actualDamage * 3;
						else
							HealthHead -= actualDamage * 3.5;
					}
			...
			else  // sides of head treated as torso
			{
				...
				if(bHasTorsoDefence){
					if(actualDamage < 14)
						HealthTorso -= actualDamage * 1.2;
					else
						HealthTorso -= actualDamage * 1.7;
		...
		else if (offset.z < 0.0)	// legs
		{
			if (offset.y > 0.0)
			{
				...
				if(hitPos == HITLOC_RightLegBack)
					HealthLegRight -= actualDamage * 2;
				else if(hitPos == HITLOC_RightLegFront)
				{
					if(bHasLegDefence){
						if(actualDamage < 14)
							HealthLegRight -= actualDamage * 1.2;
						else
							HealthLegRight -= actualDamage * 1.7;
					}
					else
						HealthLegRight -= actualDamage * 2;
				}
			...
			else
			{
				...
				if(hitPos == HITLOC_LeftLegBack)
					HealthLegLeft -= actualDamage * 2;
				else if(hitPos == HITLOC_LeftLegFront)
				{
					if(bHasLegDefence){
						if(actualDamage < 14)
							HealthLegLeft -= actualDamage * 1.2;
						else
							HealthLegLeft -= actualDamage * 1.7;
					}
					else
						HealthLegLeft -= actualDamage * 2;
				}
                        ...
                        bool bPlayAnim)
{
	...
	if(bInvincibleForNpc && instigatedBy != DeusExPlayer(GetPlayerPawn()))
		return;
...
{
	PlaySound(sound'BodyThud', SLOT_Interact,,,, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
}
...
function Gasp()
{
	PlaySound(sound'MaleGasp', SLOT_Pain,,,, RandomPitch()*DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
}
...
function PlayDyingSound()
{
	...
	if(Die != None)
	{
		PlaySound(Die, SLOT_Pain,,,, RandomPitch()*DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
	}
	else if(!IsA('Animal') && !IsA('Robot') && !bIsFemale)
	{
		num = Rand(12);
		...
		SetDistressTimer();
		PlaySound(Sound(DynamicLoadObject(soundFileName, class'Sound')), SLOT_Pain,,,, RandomPitch()*DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
	}
...
function PlayTakeHitSound(int Damage, name damageType, int Mult)
{
	...
	if (Level.TimeSeconds - LastPainSound < 1.25)
		return;
	if (Damage <= 0)
		return;
	...
	if(bMetal)
	{
		hitSound = HelmetHitSound;
	}
	else if(HitSound1 != None && HitSound2 != None)
	{
		if (Damage <= 20)
			hitSound = HitSound1;
		else
...
//	volume      = (speedFactor+0.7)*massFactor;
	...
	pitch *= DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier();
...
//	ClientMessage("PlayDying()");
	    ...
	    (damageType == 'Poison') || (damageType == 'PoisonEffect'))
	{
		bStunned = True;
		if (bIsFemale)
			PlaySound(Sound'FemaleUnconscious', SLOT_Pain,,,, RandomPitch()*DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
		else
			PlaySound(Sound'MaleUnconscious', SLOT_Pain,,,, RandomPitch()*DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
	}
	else
	{
		bStunned = False;
...
*/
	...
	UpdateAmrita(deltaTime);
	UpdateEmpField(deltaTime);
}
...
Begin:
	...
	if ((Weapon != None) && bKeepWeaponDrawn && (Weapon.CockingSound != None) && !bSeekPostCombat)
		PlaySound(Weapon.CockingSound, SLOT_None,,, 1024, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
	Acceleration = vect(0,0,0);
	if (!PickDestination())
		Goto('DoneSeek');
...
State Attacking
{
	...
	function Tick(float deltaSeconds)
	{
		...
		if ((FRand() < 0.035) && (!IsA('Robot')) && (!IsA('Animal')))
		{
                    PlayTargetAcquiredSound();
		}
...
ContinueFire:
	while (!ReadyForWeapon())
	{
		...
		if (/*bRunToAttackingEnemy && */(!IsWeaponReloading() || bCrouching))
			TurnToward(enemy);
		else
			Sleep(0);
	}
...
state FallingState 
{
	...
	function Landed(vector HitNormal)
	{
		...
		if ( (Velocity.Z < -0.8 * minJumpZ) || bUpAndOut)
		{
			...
			if ( !FootRegion.Zone.bWaterZone )
				PlaySound(Land, SLOT_Interact, FMin(20, landVol),,, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
		}
		else if ( Velocity.Z < -0.8 * JumpZ )
			PlayLanded(Velocity.Z);
	}
...
function UpdateEmpField(float deltatime)
{
		local Actor target;
		local float mindist;
		...
		if(EmpFieldRadius <= 1 || empTimer < 0.8)
			return;
        ...
        target = FindValidEmpTarget();
		
		if (target != None)
		{
            mindist = VSize(self.Location - target.Location);
       		
			if (mindist < EmpFieldRadius)
			{
				target.PlaySound(Sound'DeusExSounds.Weapons.EMPGrenadeExplode', SLOT_None,,,, 1.0*Player.GetSoundPitchMultiplier());
				
				if(target.IsA('SpyDrone'))
					SpyDrone(target).bEMPed = True;
				else
                	target.TakeDamage(30, self, target.Location, vect(0,0,0),'EMP');       
                ...
                else if(target.IsA('DeusExPlayer'))
                	DeusExPlayer(target).SpawnEmpHitEffect();
			}
		}
        ...
        if (bFound)
		{
			for(i=0; i<30; i++)
			{
				...
				if(proj != None && (VSize(self.Location - proj.Location) < EmpFieldRadius))
				{
					proj.bEMPed = True;
					proj.SpawnEmpHitEffect();
					proj.PlaySound(Sound'ProdFire', SLOT_None,,,, 2.0*Player.GetSoundPitchMultiplier());
				}
				else
					break;
			}
...
simulated function Actor FindValidEmpTarget()
{
   local Actor mytarget, target;
   local float dist, mindist;
   
   target = None;
   mindist = EmpFieldRadius;
   
   ForEach RadiusActors(class'Actor', mytarget, mindist, self.Location)
   {
      if (IsAValidTarget(myTarget))
      {      
		dist = VSize(self.Location - mytarget.Location);
...
{
		if (mytarget.IsA('DeusExPlayer') && !(DeusExPlayer(mytarget).IsInState('Dying') || DeusExPlayer(mytarget).IsInState('Paralyzed') || DeusExPlayer(mytarget).IsInState('Interpolating'))) 
	    {
	    	if (self.GetPawnAllianceType(DeusExPlayer(mytarget)) != ALLIANCE_Hostile || DeusExPlayer(mytarget).Energy < 5)
	             	return false;
	        else
	        		return true;
	    }
        ...
        {     
        	if (Robot(mytarget).GetPawnAllianceType(self) != ALLIANCE_Hostile)
             	return false;
        ...
        if(mytarget.IsA('ScriptedPawn') && mytarget != self)
		{
			if (ScriptedPawn(mytarget).GetPawnAllianceType(self) != ALLIANCE_Hostile)
             	return false;
        ...
        {
        	if (/*GrenadeProjectile(mytarget).bArmed && */!GrenadeProjectile(mytarget).bDisabled) 
            	return true;
        	else  
            	return false;
        }
...
function bool FindProjectiles(out DeusExProjectile list[30])
{
	...
	foreach AllActors(class'DeusExProjectile', proj)
	{
		...
		if (bValidProj)
		{
			...
			{
				dist = VSize(self.Location - proj.Location);
...
defaultproperties
{	 
	 bRunToAttackingEnemy=True
     MeleeDamageThreshold=0
     bHasHelmet=False
     bHasMask=False
     bHasLegDefence=False
     HelmetHitSound=Sound'DeusExSounds.Generic.ArmorRicochet'
     AnimSequence=Idle1
