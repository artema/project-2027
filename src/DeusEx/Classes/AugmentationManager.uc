...
var localized String msgLowPower;
...
function BoostAugs(bool bBoostEnabled, Augmentation augBoosting, optional int score)
{
	local Augmentation anAug;
...
*/
   ...
   {
	//augkey = AugLocs[PowerAug.AugmentationLocation].augCount + AugLocs[PowerAug.AugmentationLocation].KeyBase;

	if(PowerAug.bIsActive)
	{
		if((energyUse / energyMult) - ((PowerAug.GetEnergyRate()/60) * deltaTime) <= energyUse)
		{
			//player.ClientMessage(SprintF(msgLowPower, PowerAug.AugmentationName));
			PowerAug.Deactivate();
		}
	}
	else
	...
	if(player.bProcessingData)
		energyUse += ((600/60) * deltaTime);
...
defaultproperties
{
     ...
     AugLocs(5)=(NumSlots=1,KeyBase=2)
     AugLocs(6)=(NumSlots=2,KeyBase=11)

     augClasses(0)=Class'DeusEx.AugSpeed'
     augClasses(1)=Class'DeusEx.AugVision'
     augClasses(2)=Class'DeusEx.AugCloak'
     augClasses(3)=Class'DeusEx.AugBallistic'
     augClasses(4)=Class'DeusEx.AugEnviro'
     augClasses(5)=Class'DeusEx.AugHealing'
     augClasses(6)=Class'DeusEx.AugLight'
     augClasses(7)=Class'DeusEx.AugMuscle'
     augClasses(8)=Class'DeusEx.AugEMPShield'
     augClasses(9)=Class'DeusEx.AugDatalink'
     augClasses(10)=Class'DeusEx.AugHeartLung'
     augClasses(11)=Class'DeusEx.AugPower'
     augClasses(12)=Class'DeusEx.AugTime'
     defaultAugs(0)=Class'DeusEx.AugLight'
     defaultAugs(1)=Class'DeusEx.AugDatalink'
     bHidden=True
     bTravel=True
}
