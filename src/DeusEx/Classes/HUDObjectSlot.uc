...
function UpdateItemText()
{
	...
	if (item != None)
	{
		if (item.IsA('DeusExWeapon'))
		{
			...
			// Ammo loaded
			if ((weapon.AmmoName != class'AmmoNone') && (!weapon.bHandToHand) && (weapon.ReloadCount != 0) && (weapon.AmmoType != None) && (weapon.AmmoNames[1] != None))
				itemText = weapon.AmmoType.beltDescription;

			// If this is a grenade
			if (weapon.bIsGrenade || weapon.bHasSpecialAmmo)
			{
				if (weapon.AmmoType.AmmoAmount > 1)
					itemText = Sprintf(CountLabel, weapon.AmmoType.AmmoAmount);
			}
		...
		else if (item.IsA('DeusExPickup') && (!item.IsA('NanoKeyRing')))
		{
			...
			if (DeusExPickup(item).NumCopies > 1)
				itemText = Sprintf(CountLabel, String(DeusExPickup(item).NumCopies));
		}
	}
}
   ...
   DrawHUDBackground(gc);
	...
	if ((item != None) && (item.Icon != None) && (!bDragging))
	{
      ...
      DrawHUDIcon(gc);
		...
		gc.DrawText(1, 41, 46, 11, item.BeltDescription);//1, 42, 42, 9

		// If there's any additional text (say, for an ammo or weapon), draw it
		if (itemText != "")
			gc.DrawText(slotIconX, itemTextPosY-3, slotFillWidth, 11, itemText);
	...
	gc.SetTextColor(colObjectNum);
	gc.DrawText(slotNumberX - 1 - 5, slotNumberY + 2, 10, 10, objectNum);
}
