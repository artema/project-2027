...
function Landed(vector HitNormal)
{
	...
	if (Velocity.Z <= -200)
	{
		...
		if (hitSound != None)
			PlaySound(hitSound, SLOT_None,,,,DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
	...
	if (bPushable && (PlayerPawn(Other)!=None) && (Other.Mass > 40))// && (Physics != PHYS_Falling))
	{
		...
		if (abs(Location.Z-Other.Location.Z) < (CollisionHeight+Other.CollisionHeight-1))
		{
			...
			if (Mass <= maxPush)
			{
				...
				{
					pushSoundId = PlaySound(PushSound, SLOT_Misc,,, 128, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
...
function Explode(vector HitLocation)
{
	...
	if (explosionRadius <= 128)
		PlaySound(Sound'SmallExplosion1', SLOT_None,,, explosionRadius*16, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
	else
		PlaySound(Sound'LargeExplosion1', SLOT_None,,, explosionRadius*16, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
...
auto state Active
{
	function TakeDamage(int Damage, Pawn EventInstigator, vector HitLocation, vector Momentum, name DamageType)
	{
		...
		if ((DamageType == 'EMP') || (DamageType == 'NanoVirus') || (DamageType == 'Shocked') || (DamageType == 'Stunned'))
			return;
		...
		else	// destroy it!
		{
			...
			{
				if (avg > 20)
					PlaySound(sound'WoodBreakLarge', SLOT_Misc,,, 512, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
				else
					PlaySound(sound'WoodBreakSmall', SLOT_Misc,,, 512, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
...
//			}
			...
			if (bExplosive)
			{
				Frag(fragType, Momentum * explosionRadius / 4, avg/15.0, avg/4 + 1);
				Explode(HitLocation);
			}
			else
				Frag(fragType, Momentum / 10, avg/15.0, avg/4 + 1);
		}
	}
}
