...
var MenuUIListBigWindow          lstEmail;
...
function CreateEmailListWindow()
{
	...
	lstEmail = MenuUIListBigWindow(winScroll.clipWindow.NewChild(Class'MenuUIListBigWindow'));
...
function CreateEmailHeaders()
{
	...
	winEmailTo      = CreateSmallMenuLabel(235, 105, "", winClient);
	winEmailSubject = CreateSmallMenuLabel( 50, 119, "", winClient);
	winEmailCC      = CreateSmallMenuLabel(235, 119, "", winClient);
}
...
//		else
	...
	tagName = StringToName(emailName);

	ProcessDeusExText(tagName);
	...
	if (emailIndex != -1)
	{
		...
		for(emailInfoIndex=0; emailInfoIndex<=emailIndex; emailInfoIndex++)
		{
			...
			emailText = winEmail.GetText();
			
			note = player.GetNote(tagName);

			if (note == None)
			{
				note = player.AddNote(emailText,, true, true);
				note.SetTextTag(tagName);
			}
...
//
	if ((winStatus != None) && (compOwner.IsA('Computers')))
		winStatus.SetText(Computers(compOwner).GetNodeAddress() $ "/" $ ComputerNodeFunctionLabel);
//
}
