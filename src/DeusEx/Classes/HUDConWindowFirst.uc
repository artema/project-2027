...
function DisplayText(string text, Actor speakingActor, optional bool bHighlight)
{
	...
	newText.SetFont(fontSpeech);
	newText.SetText(text);

	// Use a different color for the player's text
	if ( DeusExPlayer(speakingActor) != None || bHighlight) 
...
defaultproperties
{
     FontName=Font'DeusExUI.FontReplicaTitle'
     fontSpeech=Font'DeusExUI.FontReplicaText'
     txtVertMargin=10
}
