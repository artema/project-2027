...
event DrawWindow(GC gc)
{
	...
	if ( weapon != None )
	{
		...
		if (weapon.ReloadCount > 1 )
		{
			...
			if (weapon.IsInState('Reload'))
				gc.DrawText(infoX, 41, 20, 9, msgReloading);
			else
				gc.DrawText(infoX, 41, 20, 9, clipsRemaining);
		}
		else
		{
			gc.DrawText(infoX, 41, 20, 9, NotAvailable);
		...
		/*if (weapon.bCanTrack)
		{
			...
			gc.DrawText(25, 58, 65, 8, weapon.TargetMessage);//25.56.65.8
		}*/
	}
}
...
function DrawBackground(GC gc)
{
	...
	//gc.DrawText(64, 15, 25, 10, AmmoLabel);
	//gc.DrawText(64, 48, 25, 10, ClipsLabel);
}
