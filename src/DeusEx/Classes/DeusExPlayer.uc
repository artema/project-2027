...
var globalconfig bool bHUDBoxVisible;
...
var globalconfig bool bStatusVisible;
...
var globalconfig bool bWeatherEnabled;
var globalconfig bool bNoResurrection;
var globalconfig bool bRPGStatVisible;
var globalconfig bool bAutosaveEnabled;
var globalconfig bool bShowItemArticles;
...
var localized String GoalFailedMsg;
...
const MaxHeroLevel = 9;
var() travel int ExperiencePoints;
var() travel int HeroLevel;
var() travel int UpgradePoints;
var() int ExperienceLevel[9];//!!1
var localized String msgHeroLevelAward;

var() bool bBotControlActive;
var() SupportBotBase aBot;
var string LoadingScrName;
var travel PerkManager PerkSystem;


var float BerserkTimer;
var bool bBerserk;



var bool bDoingSilence;
var() float SilenceDuration;
var float SilenceTimer;
var int OrigSoundVolume;
var int OrigVoiceVolume;

var rotator FixedViewRotation;

var() float BurnTime;

var() travel bool bProcessingData;
var() travel bool bProcessingDataMax;

var bool bPaused;

var globalconfig String InternetDomain;
var globalconfig String InternetReadScript;
var globalconfig String InternetTimeout;

var bool bMusicSaved;

var localized String	DefaultTruePlayerName;
var localized String labelAutosave;

var travel bool		bCheatsActivated;

var bool bInCarcassMenu;

var() float DifficultyAmount1;
var() float DifficultyAmount2;
var() float DifficultyAmount3;
var() float DifficultyAmount4;
...
replication
{
    ...
    reliable if ((Role == ROLE_Authority) && (bNetOwner))
        AugmentationSystem, SkillSystem, SkillPointsTotal, SkillPointsAvail, inHand, inHandPending, KeyRing, Energy, bSpyDroneActive, DXGame, bBuySkills, drugEffectTimer, killProfile, PerkSystem;
...
function InitializeSubSystems()
{
	...
	// install the perk system if not found
	if (PerkSystem == None)
	{
		PerkSystem = Spawn(class'PerkManager', Self);
		PerkSystem.CreatePerks(Self);
	}
	else
	{
		PerkSystem.SetPlayer(Self);
	}
...
exec function StartNewGame(String startMap)
{
	...
	if (startMap == "")
		Level.Game.SendPlayer(Self, "01_Intro");		// TODO: Must be stored somewhere!
	else
...
function StartCredits()
{
	...
	//DeleteSaveGameFiles();
	//bStartingNewGame = True;
	Level.Game.SendPlayer(Self, "07_Credits");
}
	...
	else
	{
		if (musicCheckTimer >= 1.0)
		{
	        ...
	        for (CurPawn = Level.PawnList; CurPawn != None; CurPawn = CurPawn.NextPawn)
	        {
	            ...
	            if ((npc != None) && (VSize(npc.Location - Location) < (10000 + npc.CollisionRadius)))
	            {
	               if ((npc.GetStateName() == 'Attacking') && (npc.Enemy == Self) && !npc.bHidden)
	               {
	                  bCombat = True;
	                  break;
	               }
...
function MaintainEnergy(float deltaTime)
{
	...
	if ((!IsInState('Dying')) && (!IsInState('Paralyzed')))
   {
      ...
      // up to 25%.
      if (!bProcessingData && (energyUse == 0) && (Energy <= MaxRegenPoint) && (Level.NetMode != NM_Standalone))
      {
...
{
	PlaySound(sound'BodyThud', SLOT_Interact,,,, GetSoundPitchMultiplier());
}
...
function bool IsHighlighted(actor A)
{
	...
	if (A != None)
	{
		...
		if (A.IsA('DeusExMover') && !DeusExMover(A).bHighlight)
			...
			return False;
		else if (A.IsA('DeusExProjectile') && !DeusExProjectile(A).bStuck && !DeusExProjectile(A).bHighlight)
			return False;
...
function int HealPlayer(int baseHealPoints, optional Bool bUseMedicineSkill)
{
	...
	{
		if (bUseMedicineSkill)
			PlaySound(sound'MedicalHiss', SLOT_None,,, 256, GetSoundPitchMultiplier());
...
function DoJump( optional float F )
{
	...
	{
		if ( Role == ROLE_Authority )
			PlaySound(JumpSound, SLOT_None, 1.5, true, 1200, (1.0 - 0.2*FRand()) * GetSoundPitchMultiplier() );
...
event HeadZoneChange(ZoneInfo newHeadZone)
{
	...
	if (newHeadZone.bWaterZone && !HeadRegion.Zone.bWaterZone)
	{
		...
		mult = SkillSystem.GetSkillLevelValue(class'SkillPower');
...
{
	if (IsInState('Interpolating') || IsInState('Dying') || IsInState('Paralyzed') || IsInCutscene())
		return True;
...
// DroneExplode
// ----------------------------------------------------------------------
simulated function DroneExplode()
{
	if (aDrone != None)
	{
		aDrone.DoExplode();
...
simulated function DroneDisabled()
{
	if (aDrone != None)
	{
		aDrone.Deactivated();
   ...
   oldFirstItem = Inventory;
	...
	if (FrobTarget != None)
	{
		...
		if (FrobTarget.IsA('NanoKey'))
		{
			if(FrobTarget.Event != '')
			{
				foreach AllActors(class 'Actor', A, FrobTarget.Event)		
					A.Trigger(FrobTarget, DeusExPlayer(GetPlayerPawn()));
			}
	...
	else
	{
		...
		// or drop the corpse
		if ((inHand != None) && inHand.IsA('SupportBotControlBase') && aBot != None)
		{
			SupportBotControlBase(inHand).AltFire(0);
			aBot = None;
			return;
		}
		else if ((inHand != None) && inHand.IsA('POVCorpse'))
...
function bool HandleItemPickup(Actor FrobTarget, optional bool bSearchOnly, optional out String errorMsg)
{
	...
	else 
	{
		...
		if (foundItem != None)
		{
			...
			else if (foundItem.IsA('DeusExWeapon') && (DeusExWeapon(foundItem).bIsGrenade || DeusExWeapon(foundItem).bHasSpecialAmmo))
			{
			...
			else if (foundItem.IsA('Weapon'))
			{
				...
				bCanPickup = bCanPickup && (FindInventoryType(FrobTarget.Class) == None || DeusExWeapon(foundItem).bIsGrenade || DeusExWeapon(foundItem).bHasSpecialAmmo);

				if (!bCanPickup)
				{
					errorMsg = Sprintf(CanCarryOnlyOne, foundItem.itemName);
	...
	if (bSlotSearchNeeded && bCanPickup)
	{
		...
		{
			errorMsg = Sprintf(InventoryFull, Inventory(FrobTarget).itemName);
...
function UpdateInHand()
{
   ...
   if (Role < ROLE_Authority)
	...
	if ((inHand != None) && (inHand.IsA('DeusExPickup')))
	{
		if(AugmentationSystem.GetClassLevel(class'AugCloak') >= 0 || UsingChargedPickup(class'AdaptiveArmor'))
		{
			DeusExPickup(inHand).ShowCamo();
		}
		else
		{
...
function bool CanBeLifted(Decoration deco)
{
	...
	if (AugmentationSystem != None)
	{
		augLevel = AugmentationSystem.GetAugLevelValue(class'AugMuscle');
		augMult = 1;
		if (augLevel >= 0)
			augMult = augLevel+2;
	}
	...
	if (SkillSystem != None)
	{
		SkillLevel = SkillSystem.GetSkillLevelValue(class'SkillPower');
		SkillMult = 0;
		if (SkillLevel >= 1)
			SkillMult = SkillLevel;
	}
		if(augMult > 1) maxLift *= augMult;
		if(SkillMult > 1) maxLift *= SkillMult;
...
exec function bool DropItem(optional Inventory inv, optional bool bDrop)
{
	...
	local int i;
	local Inventory nextInvItem, invitem, startItem;
	...
	if (item != None)
	{
		...
		if (item.IsA('DeusExPickup'))
		{
			...
			// Remove camo
			if (DeusExPickup(item).bItemInvisible)
				DeusExPickup(item).HideCamo();
				...
				if (POVCorpse(item).carcClassString != "")
				{
					...
					if (carcClass != None)
					{
						...
						if (carc != None)
						{
							...
							invitem = POVCorpse(item).TempInventory;
							startItem = invitem;					
							do
							{
								nextInvItem = invitem.Inventory;					
								if(invitem != None) invitem.SetOwner(carc);					
								invitem = nextInvItem;
							}
							until ((invitem == None) || (invitem == startItem));	
							carc.Inventory = POVCorpse(item).TempInventory;						
							POVCorpse(item).TempInventory = None;
...
// ----------------------------------------------------------------------
exec function ToggleRPGStatDisplay()
{
	local DeusExRootWindow root;

	bRPGStatVisible = !bRPGStatVisible;
...
exec function ToggleHUDBox()
{
	local DeusExRootWindow root;

	bHUDBoxVisible = !bHUDBoxVisible;
...
event PlayerCalcView( out actor ViewActor, out vector CameraLocation, out rotator CameraRotation )
{
	...
	if (bSpyDroneActive)
	{
		...
		{
			// First-person view.
			CameraRotation = FixedViewRotation;
			CameraLocation = Location;
			CameraLocation.Z += EyeHeight;
			CameraLocation += WalkBob;
			
			rot = Rotation;
			rot.Yaw = FixedViewRotation.Yaw;			
			SetRotation(rot);
...
function DeusExNote AddNote( optional String strNote, optional Bool bUserNote, optional bool bShowInLog, optional bool bNoLogSound)
{
...
function GoalFailed( Name goalName )
{
	local DeusExGoal goal;
	...
	if ( goal != None )
	{
		...
		{
			goal.SetCompleted();
			DeusExRootWindow(rootWindow).hud.msgLog.PlayLogSound(Sound'Menu_Incoming');

			ClientMessage(GoalFailedMsg);
		}
	}
}
...
// SetMaxLogLines()
// ----------------------------------------------------------------------
function SetMaxLogLines(Byte newMaxLogLines)
{
...
function CreateKillerProfile( Pawn killer, int damage, name damageType, String bodyPart )
{
	...
	switch( damageType )
	{
		case 'AutoShot':
			...
			{
				killProfile.activeSkill = class'SkillElectronics'.Default.skillName;
				killProfile.activeSkillLevel = pkiller.SkillSystem.GetSkillLevel(class'SkillElectronics');
			}
			break;
		case 'PoisonEffect':
	...
	if ( killProfile.methodStr ~= NoneString )
	{
		...
		if (( killer != None ) && (proj != None) && (!(proj.itemName ~= "")) )
		{
			if ( (LAM(myProjKiller) != None) && (LAM(myProjKiller).bProximityTriggered) )
			{
				...
				killProfile.myActiveSkill = class'SkillWeaponHeavy'.Default.skillName;
				if ( SkillSystem != None )
					killProfile.myActiveSkillLevel = SkillSystem.GetSkillLevel(class'SkillWeaponHeavy');
				else
					killProfile.myActiveSkillLevel = 0;
			}
			else
		...
		if ( ( instigatedBy != None ) && (instigatedBy.IsA('DeusExPlayer')) )
		{
			...
			if ((DeusExPlayer(instigatedBy).Weapon != None) && ( DeusExPlayer(instigatedBy).Weapon.class == class'WeaponRifle' ))
			{
				...
				if ( (dxw != None ) && ( !dxw.bZoomed ))
					actualDamage *= 0.5; // Reduce damage if we're not using the scope
			}
...
function float CalculatePlayerVisibility(ScriptedPawn P)
{
	...
	if ((P != None) && (AugmentationSystem != None))
	{
		if (P.IsA('Robot'))
		{
			if (IsInvisibleForRobots())
				vis = 0.0;
		}
		else
		{
			if (IsInvisibleForCreatures())
				vis = 0.0;
		}
...
exec function DXDumpInfo()
{
	...
	if (PerkSystem != None)
	{
		perk = PerkSystem.FirstPerk;
		while (perk != None)
		{
			if (perk.PerkName != "" && perk.bInstalled)
				log("    "$perk.PerkName$" installed");
...
function MultiplayerTick(float DeltaTime)
{
   ...
   lastRefreshTime = 0;
	...
	bCheatsEnabled = bCheatsActivated;
...
/*
	if (bWeatherEnabled)
	{
		foreach AllActors(class'WeatherEffects', WE)

		if (WE.bUseDistance)
		{
			if (VSize(WE.Location - Location) > WE.MaxDistance)
				WE.bNotActive = True;
			else
				WE.bNotActive = False;
		}
...
//!!1
	...
	if(HeroLevel<MaxHeroLevel)
	{
		...
		for(i=0;i<=HeroLevel;i++){
			TotalPoints += ExperienceLevel[i];
		}

		if(ExperiencePoints>=TotalPoints){
			DeusExRootWindow(rootWindow).hud.msgLog.PlayLogSound(Sound'Menu_BuySkills');
			HeroLevel++;
			UpgradePoints++;
			ClientMessage(Sprintf(msgHeroLevelAward, HeroLevel-1));
		}
	}

	if(inHand != None && inHand.IsA('SupportBotControlBase')){
		bBotControlActive=True;
	}
	else{
...
//////////////////////////////////////////////////////////////////////////////////
	...
	if(bDoingSilence)
	{
		if(SilenceTimer >= SilenceDuration)
			...
			SilenceTimer += DeltaTime*10;
	
			tmpvolume = int(float(OrigSoundVolume) / (SilenceDuration - SilenceTimer));
			
			if(tmpvolume > OrigSoundVolume || tmpvolume < 0)
				tmpvolume = OrigSoundVolume;
				
			SetInstantSoundVolume(tmpvolume);	
			ClientMessage(tmpvolume $ "(" $ SilenceTimer $ ")");
		}
	}
}
...
function SetLoadScr(string scr)
{
	LoadingScrName = scr;
...
simulated function DoExplosionSilence()
{
	if(bDoingSilence || PerkSystem.CheckPerkState(class'PerkFire')){		
		return;
	}
	...
	OrigSoundVolume = int(ConsoleCommand("get ini:Engine.Engine.AudioDevice SoundVolume"));
	OrigVoiceVolume = int(ConsoleCommand("get ini:Engine.Engine.AudioDevice SpeechVolume"));
	SetInstantSoundVolume(0);
	SetInstantSpeechVolume(OrigSoundVolume);

	PlaySound(Sound'GameMedia.SilenceSound', SLOT_Talk);

}

simulated function EndExplosionSilence()
{
	bDoingSilence = False;
	SilenceTimer = 0;
	SetInstantSoundVolume(OrigSoundVolume);
	SetInstantSpeechVolume(OrigVoiceVolume);
}

function bool IsInvisibleForCreatures()
{
	return ((AugmentationSystem.GetClassLevel(class'AugCloak') >= 0) || (UsingChargedPickup(class'AdaptiveArmor')));
}

function bool IsInvisibleForRobots()
{
	return ((AugmentationSystem.GetClassLevel(class'AugCloak') >= 1) || (UsingChargedPickup(class'AdaptiveArmor')));
}
...
{
	ClientFlash(5, vect(300,300,300));
	bProcessingData = False;	
}

function StartProcessingDataMax()
{
	local DeusExRootWindow root;
	...
	ClientFlash(10, vect(500,500,500));
	bProcessingDataMax = True;
	
	if ((root != None) && (root.hud != None))
	{
		if (root.hud.background == None)
		{
			root.hud.SetBackground(Texture(DynamicLoadObject("Extras.Matrix_A00", class'Texture')));
			root.hud.SetBackgroundSmoothing(True);
			root.hud.SetBackgroundStretching(False);
...
function StopProcessingDataMax()
{
	local DeusExRootWindow root;
...
function AutoSaveGame(string saveTitle)
{
	...
	SaveGame(0, Sprintf(labelAutosave, saveTitle));
	SetLoadScr(GetLevelInfo().CurrentMapScreen);
}

function bool IsInCutscene()
{
	return flagBase.GetBool('PlayerInCutscene');
}

function float GetBadLuck()
{
	local float value;
	//Will be 0.5 and 1.5 for hard and realistic and 0.0 for easy and normal
	//FRand() <= GetBadLuck()
	
	value = FMax(0.0, CombatDifficulty - 1.0);
...
defaultproperties
{
	 DifficultyAmount1=0.25
	 DifficultyAmount2=0.5
	 DifficultyAmount3=1.5
	 DifficultyAmount4=2.5
	 CombatDifficulty=2.5
     BurnTime=15.0
     SilenceDuration=6.0
     aBot=None
     bBotControlActive=False
     bNoResurrection=True     
     ...
     UpgradePoints=0
     ExperienceLevel(0)=0
     ExperienceLevel(1)=200
     ExperienceLevel(2)=300
     ExperienceLevel(3)=400
     ExperienceLevel(4)=500
     ExperienceLevel(5)=600
}