...
function SetCannister(AugmentationCannister newAugCan)
{
	...
	if(augCan.GetAugmentation(0) != None)
	{
	...
	{
		btnAug1.bIsEmpty = True;
		btnAug1.SetSensitivity(False);
	}
	
	if(augCan.GetAugmentation(1) != None)
	{
	...
	{
		btnAug2.bIsEmpty = True;
		btnAug2.SetSensitivity(False);
	}

	if(augCan.GetAugmentation(0) != None && augCan.GetAugmentation(1) != None)
		augDesc = btnAug1.GetAugDesc() $ "|n" $ btnAug2.GetAugDesc();
	else if(augCan.GetAugmentation(0) == None && augCan.GetAugmentation(1) != None)
		augDesc = btnAug2.GetAugDesc();
	else if(augCan.GetAugmentation(0) != None && augCan.GetAugmentation(1) == None)
		augDesc = btnAug1.GetAugDesc();
	else
		augDesc = "";
