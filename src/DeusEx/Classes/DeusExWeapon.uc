...
var() bool bIsUniqueWeapon;
var() bool bCannotBePickedUp;

var() Sound	SilencedSound;
...
var() int NumConeSlugs;

var() bool bHasSpecialAmmo;

var() float NonHumanDamageMultiplier;
...
{
   Owner.PlaySound(LockedSound, SLOT_None,,,, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());
}
...
//		}
		...
		if (Item.PickupMessageClass == None)
		{
			if(Item.itemArticle != "" && Item.itemArticle != " " && DeusExPlayer(GetPlayerPawn()).bShowItemArticles)
				...
				P.ClientMessage(Item.PickupMessage @ Item.itemName, 'Pickup');
		}
		else
...
simulated function float CalculateAccuracy()
{
   ...
   weapskill = GetWeaponSkill();
	...
	if(player.PerkSystem.CheckPerkState(class'PerkMarksman'))
		checkwithperk = True;
	...
	if (ClipCount < ReloadCount)
	{
      ...
      if ( (Level.Netmode == NM_Standalone) || ( (Player != None) && (Player.PlayerIsClient()) ) )
      {
         if (bCanTrack)
         {
            ...
            case LOCK_Range:
               TargetMessage = msgLockRange @ Int(TargetRange/52.5) @ msgRangeUnit;
               LockTimer -= deltaTime;
               break;
            ...
            case LOCK_Locked:
               ...
               TargetMessage = msgLockLocked @ Int(TargetRange/52.5) @ msgRangeUnit;
...
simulated function Projectile ProjectileFire(class<projectile> ProjClass, float ProjSpeed, bool bWarn)
{
	...
	// AugMuscle increases our speed (distance) if hand to hand
	mult = 1.0;
	...
	for (i=0; i<numProj; i++)
	{
      ...
      if ((i > 0) && (Level.NetMode != NM_Standalone))
		...
		if (( Level.NetMode == NM_Standalone ) || ( Owner.IsA('DeusExPlayer') && DeusExPlayer(Owner).PlayerIsListenClient()) )
		{
			...
			if (proj != None)
			{
				// AugMuscle increases our damage as well
				proj.Damage *= mult;
...
simulated function bool UpdateInfo(Object winObject)
{
	...
	if (AreaOfEffect == AOE_Cone)
	{
		...
		{
			if (Level.NetMode != NM_Standalone)
				dmg = Default.mpHitDamage * NumConeSlugs;
			else
...
{
	if(DeusExPlayer(Owner).PerkSystem.CheckPerkState(class'PerkDoubleMods'))
		return (ModReloadTime == -0.5);
	else
		return (ModReloadTime == -0.25);
}
...
Begin:
	...
	if (AmmoType.AmmoAmount > 0)
	{
		...
		else
		{
			...
			Owner.PlaySound(CockingSound, SLOT_None,,, 1024, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());		// CockingSound is reloadbegin
			PlayAnim('ReloadBegin', GetReloadingAnimRate());
			NotifyOwner(True);
			FinishAnim();
			LoopAnim('Reload');
			Sleep(GetReloadTime());
			Owner.PlaySound(AltFireSound, SLOT_None,,, 1024, DeusExPlayer(GetPlayerPawn()).GetSoundPitchMultiplier());		// AltFireSound is reloadend
...
function bool SupressSounds()
{
	return (DeusExPlayer(Owner) != None && DeusExPlayer(Owner).flagBase.GetBool('PlayerInCutscene'));
}

defaultproperties
{
	 NonHumanDamageMultiplier=1.0
	 NumConeSlugs=15
     bReadyToFire=True
     LowAmmoWaterMark=10
     NoiseLevel=1.000000
