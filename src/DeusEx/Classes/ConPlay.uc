...
{
	if (GetCurrentResolutionWidth() <= 1280 || GetCurrentResolutionHeight() <= 960)
...
function int GetCurrentResolutionHeight()
{
	...
	resX      = InStr(CurrentRes,"x");
	resHeight  = int(Right(CurrentRes, resX));
...
function EEventAction SetupEventSpeechPost( ConEventSpeech event, out String nextLabel )
{
	...
	{			
		...
		{
			if(event.speechFont == SF_Computer)
				bHighlight = True;
			...
			{	
				...
				conWinFirst.DisplayText( speech, currentSpeaker, bHighlight );
			}
			else
			{
				// Clear the window
				...
				conWinThird.DisplayText( speech, currentSpeaker, bHighlight );
			}
		}
	}
...
function EEventAction SetupEventChoice( ConEventChoice event, out String nextLabel )
{
	...
	if ( choiceCount > 0 ) 
	{
		...
		conWinThird.ShowCredits(player.Credits);	
	}
	else
		nextAction = EA_NextEvent;
...
defaultproperties
{
     ...
     ConversationSpeechFonts(0)=Font'DeusExUI.FontBarkText'
     ConversationSpeechFonts(1)=Font'DeusExUI.FontConversationLarge'
     ConversationNameFonts(0)=Font'DeusExUI.FontBarkName'
