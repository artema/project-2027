...
event InitWindow()
{
	...
	upperConWindow = TileWindow(NewChild(Class'TileWindow'));
	upperConWindow.SetBackground(conTexture);
	upperConWindow.SetBackgroundStyle(DSTY_Modulated);
	upperConWindow.SetOrder(ORDER_UpThenLeft);
	upperConWindow.SetChildAlignments(HALIGN_Full, VALIGN_Top);
	upperConWindow.SetMargins(2, 2);
	upperConWindow.MakeWidthsEqual(False);
	upperConWindow.MakeHeightsEqual(False);
	upperConWindow.SetTileColorRGB(0, 0, 0);
...
event ConfigurationChanged()
{
	...
	upperConPosY = newHeight - 30;
	upperConWindow.ConfigureChild(75, upperConPosY, width - 150, newHeight);
...
function DisplayText(string text, Actor speakingActor, optional bool bHighlight)
{
	if (winSpeech == None ) 
		CreateSpeechWindow();

	winSpeech.SetSpeech(text, speakingActor, bHighlight);
}
...
function DestroyChildren()
{
	...
	win = upperConWindow.GetTopChild();
	while( win != None )
	{
		win.Destroy();
		win = upperConWindow.GetTopChild();
