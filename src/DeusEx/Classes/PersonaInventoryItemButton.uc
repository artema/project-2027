...
var Color colWeaponToolTrue;
var Color colWeaponToolFalse;
...
{	
	...
	if ( !bDragging )
	{
		...
		if (anItem.IsA('DeusExAmmo') || anItem.IsA('DeusExWeapon'))
		{
			...
			if ((weapon != None) && weapon.bIsGrenade && (weapon.AmmoType != None) && (weapon.AmmoName != class'AmmoNone'))
			{
				str = Sprintf(CountLabel, String(weapon.AmmoType.AmmoAmount));
			}
			else if ((weapon != None) && weapon.bHandToHand && (weapon.AmmoType != None) && (weapon.AmmoName != class'AmmoNone'))
			{
			...
			else if ((weapon != None) && (!weapon.bHandToHand) && weapon.AmmoNames[1] != None)
			{
...
function HighlightItem(bool bNewToggle)
{
	if (bNewToggle)
		fillMode = FM_WeaponToolTrue;
	else
		fillMode = FM_WeaponToolFalse;
}
...
function SetFillColor()
{
	switch(fillMode)
	{
		case FM_WeaponToolTrue:
			fillColor = colWeaponToolTrue;
			break;
		case FM_WeaponToolFalse:
			fillColor = colWeaponToolFalse;
			break;
...
defaultproperties
{
     ...
     colDragGood=(G=255)
     colDragBad=(R=255)
     colWeaponToolTrue=(R=32,G=32,B=128)
     colWeaponToolFalse=(R=128,G=32,B=32)
