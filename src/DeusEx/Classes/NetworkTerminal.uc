...
function CreateHackWindow()
{
	...
	skillLevelValue = player.SkillSystem.GetSkillLevelValue(class'SkillElectronics');
	skillLevel      = player.SkillSystem.GetSkillLevel(class'SkillElectronics');
...
function bool AreSpecialOptionsAvailable(optional bool bCheckActivated)
{
	...
	for (i=0; i<ArrayCount(Computers(compOwner).specialOptions); i++)
	{
		if (Computers(compOwner).specialOptions[i].Text != "")
		{
			...
			{
				if(Computers(compOwner).PlayerCanAccess(Computers(compOwner).specialOptionsAccessLevel[i]))
				{
